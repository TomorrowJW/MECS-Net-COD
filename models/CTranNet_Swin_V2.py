...
class Decoder(nn.Module):
    ...
    def forward(self,  o1, o2, o3, o4): #[b,64,96,96],[b,128,48,48],[b,256,24,24],[b,256,12,12]
        ...
        s3_up = self.decoder3_0[0](torch.cat((torch.cat((f4,b4),1)*self.upsample4(o3) + self.upsample4(o3),s5_up),1))
        ...
        s2_up = self.decoder2_0[0](torch.cat((torch.cat((f3,b3),1)*self.upsample2(o2) + self.upsample2(o2),s5_up),1))
        ...
        s1_up = self.decoder1_0[0](\
            torch.cat((torch.cat((self.upsample2(f2),self.upsample2(b2)),1)*self.upsample2(o1) + self.upsample2(o1),self.upsample2(s5_up)),1))
...
class CL_Model(nn.Module):
    def __init__(self):
        ...
        self.conv4 = BasicConv2d(1024, 256, 1, stride=1, padding=0)
    ...
    def forward(self, rgb):  # rgb:[b,3,416,416], dct:[b,192,48,48]
        ...
        return s1,s2,s3,s4,s5,e1,e2,e3,e4,e5#,fc2,bc2,fc3,bc3,fc4,bc4,fc5,bc5

    def initialize_weights(self):  # 加载预训练模型权重，做初始化
        self.swin_rgb.load_state_dict(
            torch.load('./pre_train/swinv2_base_patch4_window12to24_192to384_22kto1k_ft.pth')['model'],
            strict=True)
        #
        #self.resnet.load_state_dict(torch.load('/home/jasonwu/1-projects/3-COD/pre_train/resnet50-19c8e357.pth'), strict=False)

        #self.res2net.load_state_dict(torch.load('/home/imi432002/JasonWu/COD/pre_train/res2net50_v1b_26w_4s-3cf99910.pth'))
